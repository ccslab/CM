...
public class CMMqttEventHandler extends CMEventHandler {
	...
	public boolean processEvent(CMEvent event) {
		...
		case CMMqttEvent.DISCONNECT:
			...
			break;
		default:
			System.err.println("CMMqttEventHandler.processEvent(), invalid event id: (" + event.getID() + ")!");
			return false;
		}
		return bRet;
	}
	...
	private boolean processCONNECT(CMMqttEvent event) {
		...
		if (CMInfo._CM_DEBUG) {
			System.out.println("CMMqttEventHandler.processCONNECT(): received " + conEvent.toString());
		}
		...
		// In MQTT v3.1.1, if the format is invalid, the server disconnects with the
		// client.
		...
		if (bRet && CMInfo._CM_DEBUG) {
			System.out.println("CMMqttEventHandler.processCONNECT(), sent " + ackEvent.toString());
		}
		if (!bRet) {
			System.err.println("CMMqttEventHandler.processCONNECT(), error to send " + ackEvent.toString());
			return false;
		}

		// to get CMMqttManager
		CMMqttManager mqttManager = (CMMqttManager) m_cmInfo.getServiceManagerHashtable().get(CMInfo.CM_MQTT_MANAGER);
		if (mqttManager == null) {
	...
	private byte validateCONNECT(CMMqttEventCONNECT conEvent) {
		...
		if (conEvent.getPacketType() != 1) {
			System.err.println(
					"CMMqttEventHandler.validateCONNECT(), packet type is not 1! : " + conEvent.getPacketType());
			return 6;
		}
		// validate flag
		if (conEvent.getFlag() != 0) {
			System.err.println(
					"CMMqttEventHandler.validateCONNECT(), fixed header flag is not 0 : " + conEvent.getFlag());
			return 6;
		}
		...
		if (!conEvent.getProtocolName().contentEquals("MQTT")) {
			System.err.println(
					"CMMqttEventHandler.validateCONNECT(), protocol name is not MQTT! : " + conEvent.getProtocolName());
			return 6;
		}
		// validate protocol level
		if (conEvent.getProtocolLevel() != 4) {
			System.err.println(
					"CMMqttEventHandler.validateCONNECT(), protocol level is not 4 : " + conEvent.getProtocolLevel());
			return 1;
		}
		// validate will flag and will qos
		if (conEvent.isWillFlag() && (conEvent.getWillQoS() > 2 || conEvent.getWillQoS() < 0)) {
			System.err.println("CMMqttEventHandler.validateCONNECT(), wrong will QoS : " + conEvent.getWillQoS());
			return 6;
		}
		if (!conEvent.isWillFlag() && (conEvent.getWillQoS() != 0)) {
			System.err
					.println("CMMqttEventHandler.validateCONNECT(), will flag is not set, " + "but will QoS is not 0!");
			return 6;
		}
		// validate will retain
		if (!conEvent.isWillFlag() && conEvent.isWillRetainFlag()) {
			System.err.println(
					"CMMqttEventHandler.validateCONNECT(), will flag is not set, " + "but will retain is set!");
			return 6;
		}
		...
		if (conEvent.isUserNameFlag() && (strUserName == null || strUserName.isEmpty())) {
			System.err.println(
					"CMMqttEventHandler.validateCONNECT(), user name flag is set, " + "but there is no user name!");
			return 4;
		}
		if (!conEvent.isUserNameFlag() && strUserName != null && !strUserName.isEmpty()) {
		...
		if (conEvent.isPasswordFlag() && (strPassword == null || strPassword.isEmpty())) {
			System.err.println(
					"CMMqttEventHandler.validateCONNECT(), password flag is set, " + "but there is no password");
			return 4;
		}
		...
		if (confInfo.isLoginScheme() && !CMDBManager.authenticateUser(strUserName, strPassword, m_cmInfo)) {
			System.err.println("CMMqttEventHandler.validateCONNECT(), user authentication failed! " + "user("
	...
	private boolean processCONNACK(CMMqttEvent event) {
		...
		if (CMInfo._CM_DEBUG) {
			System.out.println("CMMqttEventHandler.processCONNACK(): received " + connackEvent.toString());
		}
		...
		if (connackEvent.getReturnCode() == (byte) 0) {
			...
			boolean bRet = mqttManager.resendSentUnAckPubrel(connackEvent.getSender(), session);
			if (!bRet) {
	...
	private boolean processPUBLISH(CMMqttEvent event) {
		...
		if (CMInfo._CM_DEBUG) {
			System.out.println("CMMqttEventHandler.processPUBLISH(): received " + pubEvent.toString());
		}
		...
		case 2:
			...
			break;
		case 3:
			bRet = processQos3(pubEvent);
			if (!bRet)
				return false;
			break;
		default:
			System.err.println("CMMqttEventHandler.processPUBLISH(), wrong QoS: " + pubEvent.getQoS());
			return false;
		}
...
//					session.setMinNumWaitedEvents(0); //for pubrel
					...
					mqttManager.publish(pubEvent.getTopicName(), pubEvent.getAppMessage(), pubEvent.getQoS(), false, false);
				}
			}
...
//			System.out.println("pub3 subscriber side");
	...
	private void processRetainEvent(CMMqttEventPUBLISH pubEvent) {
		...
		if (pubEvent.getAppMessage().isEmpty()) {
			...
			if (CMInfo._CM_DEBUG) {
				if (oldEvent != null) {
					System.out.println("CMMqttEventHandler.processRetainEvent(), " + "deleted retain event "
							+ oldEvent.toString());
				} else {
					System.err.println("CMMqttEventHandler.processRetainEvent(), " + "no retain event with topic \""
							+ pubEvent.getTopicName() + "\"!");
				}
			}
		} else {
			...
			oldEvent = retainHashtable.put(pubEvent.getTopicName(), pubEvent);
			if (CMInfo._CM_DEBUG) {
				if (oldEvent != null) {
					System.out.println("CMMqttEventHandler.processRetainEvent(), " + "old retain event "
							+ oldEvent.toString() + " is " + "replaced by the new retain event.");
				}

				System.out.println("CMMqttEventHandler.processRetainEvent(), " + "added new retained event "
						+ pubEvent.toString());
			}
	...
	private boolean isDuplicatePUBLISH(CMMqttEventPUBLISH pubEvent) {
		...
		if (session.findRecvUnAckPublish(nPacketID) != null) {
			System.err.println("CMMqttEventHandler.isDuplicatePUBLISH(), event with packet ID (" + nPacketID
					+ ") is received and still in use in the recv-unack-publish list!");
			return true;
		}
...
//		System.out.println("send pubrec3");
		...
		// send ack to the PUBLISH sender
		boolean bRet = false;
		bRet = CMEventManager.unicastEvent(recEvent, strReceiver, m_cmInfo);
		if (bRet && CMInfo._CM_DEBUG) 
		{
			System.out.println("CMMqttEventHandler.sendPUBREC3(): Ok " + recEvent.toString());
		}
		if (!bRet) 
		{
			System.err.println("CMMqttEventHandler.sendPUBREC3(): FAILED! " + recEvent.toString());
			return false;
		}
	...
	private boolean sendPUBREC(CMMqttEventPUBREC recEvent) {
		...
		// send ack to the PUBLISH sender
		boolean bRet = false;
		String strPublisher = recEvent.getSender();
	...
	private boolean sendPUBREL(CMMqttEventPUBREL relEvent) {
		...
		if (bRet && CMInfo._CM_DEBUG) 
		{
			System.out.println("CMMqttEventHandler.sendPUBREL(): Ok " + relEvent.toString());
		}
		if (!bRet) 
		{
			System.err.println("CMMqttEventHandler.sendPUBREL(): FAILED! " + relEvent.toString());
			return false;
		}
	...
	private boolean sendPUBCOMP(CMMqttEventPUBCOMP compEvent) {
		...
		if (bRet && CMInfo._CM_DEBUG) 
		{
			System.out.println("CMMqttEventHandler.sendPUBREL(): Ok " + compEvent.toString());
		}
		if (!bRet) 
		{
			System.err.println("CMMqttEventHandler.sendPUBREL(): FAILED! " + compEvent.toString());
			return false;
		}
	...
	private boolean processPUBACK(CMMqttEvent event) {
		...
		if (CMInfo._CM_DEBUG) 
		{
			System.out.println("CMMqttEventHandler.processPUBACK(), received " + pubackEvent.toString());
		}
		...
		if (strSysType.equals("CLIENT")) {
			...
			System.err.println("CMMqttEventHandler.processPUBACK(), wrong system type! (" + strSysType + ")");
			return false;
		}
		...
		if (bRet && CMInfo._CM_DEBUG) {
			System.out.println("CMMqttEventHandler.processPUBACK(), deleted PUBLISH event " + "with packet ID ("
					+ nPacketID + ").");
		}
		if (!bRet) {
			System.err.println("CMMqttEventHandler.processPUBACK(), error to delete PUBLISH " + "event with packet ID ("
					+ nPacketID + ")!");
			return false;
		}
	...
	private boolean processPUBREC(CMMqttEvent event) {
		...
		if (CMInfo._CM_DEBUG) {
			System.out.println("CMMqttEventHandler.processPUBREC(), received " + recEvent.toString());
		}
		...
		if (strSysType.equals("CLIENT")) {
			...
			System.err.println("CMMqttEventHandler.processPUBREC(), wrong system type! (" + strSysType + ")");
			return false;
		}
		...
		boolean bRet = session.removeSentUnAckPublish(nPacketID);
		if (bRet && CMInfo._CM_DEBUG) {
			System.out.println("CMMqttEventHandler.processPUBREC(), deleted PUBLISH event " + "with packet ID ("
					+ nPacketID + ") from the sent-unack-publish list.");
		}
		if (!bRet) {
			System.err.println("CMMqttEventHandler.processPUBREC(), error to delete " + "PUBLISH event with packet ID ("
					+ nPacketID + ") from the sent-unack-publish list !");
			return false;
		}
		...
		if (bRet && CMInfo._CM_DEBUG) {
			System.out.println("CMMqttEventHandler.processPUBREC(), added PUBREC event " + "with packet ID ("
					+ nPacketID + ") to the recv-unack-pubrec list.");
		}
		if (!bRet) {
			System.err.println("CMMqttEventHandler.processPUBREC(), error to add " + "PUBREC event with packet ID ("
					+ nPacketID + ") to the recv-unack-pubrec list!");
			return false;
		}
		...
		if (bRet && CMInfo._CM_DEBUG) {
			System.out.println("CMMqttEventHandler.processPUBREC(), sent " + relEvent.toString());
		}
		if (!bRet) {
			System.err.println("CMMqttEventHandler.processPUBREC(), error to send " + relEvent.toString());
			return false;
		}
...
//		}
		...
		if (CMInfo._CM_DEBUG) {
			System.out.println("CMMqttEventHandler.processPUBREL(), received " + relEvent.toString());
		}
		...
		if (strSysType.equals("CLIENT")) {
			...
			System.err.println("CMMqttEventHandler.processPUBREL(), wrong system type! (" + strSysType + ")");
			return false;
		}
		...
		if (bRet && CMInfo._CM_DEBUG) {
			System.out.println("CMMqttEventHandler.processPUBREL(), deleted PUBLISH event " + "with packet ID ("
					+ nPacketID + ") from the recv-unack-publish list.");
		}
		if (!bRet) {
			System.err.println("CMMqttEventHandler.processPUBREL(), error to delete " + "PUBLISH event with packet ID ("
					+ nPacketID + ") from the " + "recv-unack-publish list!");
			return false;
		}
		...
		if (bRet && CMInfo._CM_DEBUG) {
			System.out.println("CMMqttEventHandler.processPUBREL(), sent " + compEvent.toString());
		}
		if (!bRet) {
			System.err.println("CMMqttEventHandler.processPUBREL(), error to send " + compEvent.toString());
			return false;
		}
...
//		}
		...
		if (CMInfo._CM_DEBUG) {
			System.out.println("CMMqttEventHandler.processPUBCOMP(), received " + compEvent.toString());
		}
		...
		if (strSysType.equals("CLIENT")) {
			...
			System.err.println("CMMqttEventHandler.processPUBCOMP(), wrong system type! (" + strSysType + ")");
			return false;
		}
		...
		if (bRet && CMInfo._CM_DEBUG) {
			System.out.println("CMMqttEventHandler.processPUBCOMP(), deleted PUBREC event " + "with packet ID ("
					+ nPacketID + ") from the recv-unack-pubrec list.");
		}
		if (!bRet) {
			System.err.println("CMMqttEventHandler.processPUBCOMP(), error to delete " + "PUBREC event with packet ID ("
					+ nPacketID + ") from the " + "recv-unack-pubrec list!");
			return false;
		}
	...
	private boolean processSUBSCRIBE(CMMqttEvent event) {
		...
		if (CMInfo._CM_DEBUG) {
			System.out.println("CMMqttEventHandler.processSUBSCRIBE(), received " + subEvent.toString());
		}

		// to check the topic/qos list
		if (subEvent.getTopicQoSList().isEmpty()) {
			System.err
					.println("CMMqttEventHandler.processSUBSCRIBE(), there is " + "no (topic, qos) pair in the event!");
			return false;
		}
		...
		if (session == null) {
			System.err.println("CMMqttEventHandler.processSUBSCRIBE(), session of client (" + strClient + ") is null!");
			return false;
		}
		...
		if (bRet && CMInfo._CM_DEBUG) {
			System.out.println("CMMqttEventHandler.processSUBSCRIBE(), sent " + ackEvent.toString());
		}
		if (!bRet) {
			System.err.println("CMMqttEventHandler.processSUBSCRIBE(), error to send " + ackEvent.toString());
			return false;
		}
	...
	private void sendRetainedEvents(String strClient, CMList<CMMqttTopicQoS> topicQoSList) {
		...
		if (session == null) {
			System.err.println("CMMqttEventHandler.sendRetainEvents(), session of client (" + strClient + ") is null!");
			return;
		}
		// get mqtt manager
		CMMqttManager mqttManager = (CMMqttManager) m_cmInfo.getServiceManagerHashtable().get(CMInfo.CM_MQTT_MANAGER);
		for (String strTopic : retainHashtable.keySet()) {
			...
			mqttManager.publishFromServerToOneClient(strTopic, strMsg, qos, bDupFlag, bRetainFlag, strClient, session);
		}
	...
	private boolean processSUBACK(CMMqttEvent event) {
		...
		if (CMInfo._CM_DEBUG) {
			System.out.println("CMMqttEventHandler.processSUBACK(), received " + ackEvent.toString());
		}
		...
		if (returnCodeVector.size() != reqSubscriptionVector.size()) {
			System.err.println("CMMqttEventHandler.processSUBACK(), # return code list (" + returnCodeVector.size()
					+ ") and # req subscription list (" + reqSubscriptionVector.size() + ") are different!");
			return false;
		}
	...
	private boolean processUNSUBSCRIBE(CMMqttEvent event) {
		...
		if (CMInfo._CM_DEBUG) {
			System.out.println("CMMqttEventHandler.processUNSUBSCRIBE(), received " + unsubEvent.toString());
		}

		// to check the topic list
		if (unsubEvent.getTopicList().isEmpty()) {
			System.err.println("CMMqttEventHandler.processUNSUBSCRIBE(), there is " + "no topic list in the event!");
			return false;
		}
		...
		if (session == null) {
			System.err
					.println("CMMqttEventHandler.processUNSUBSCRIBE(), session of client (" + strClient + ") is null!");
		} else {
		...
		if (bRet && CMInfo._CM_DEBUG) {
			System.out.println("CMMqttEventHandler.processUNSUBSCRIBE(), sent " + unsubAckEvent.toString());
		}
		if (!bRet) {
			System.err.println("CMMqttEventHandler.processUNSUBSCRIBE(), error to send " + unsubAckEvent.toString());
			return false;
		}
	...
	private boolean processUNSUBACK(CMMqttEvent event) {
		...
		if (CMInfo._CM_DEBUG) {
			System.out.println("CMMqttEventHandler.processUNSUBACK(), received " + unsubAckEvent.toString());
		}
		return true;
	}
	...
	private boolean processPINGREQ(CMMqttEvent event) {
		...
		if (CMInfo._CM_DEBUG) {
			System.out.println("CMMqttEventHandler.processPINGREQ(), received from (" + reqPingEvent.getSender() + ")");
		}
		...
		if (bRet && CMInfo._CM_DEBUG) {
			System.out.println(
					"CMMqttEventHandler.processPINGREQ(), sent PINGRESP to (" + reqPingEvent.getSender() + ")");
		}
		if (!bRet) {
			System.err.println("CMMqttEventHandler.processPINGREQ(), error to send " + "PINGRESP to ("
	...
	private boolean processPINGRESP(CMMqttEvent event) {
		...
		if (CMInfo._CM_DEBUG) {
			System.out
					.println("CMMqttEventHandler.processPINGRESP(), received from (" + resPingEvent.getSender() + ")");
		}
		return true;
	}
	...
	private boolean processDISCONNECT(CMMqttEvent event) {
		...
		if (CMInfo._CM_DEBUG) {
			System.out.println("CMMqttEventHandler.processDISCONNECT(), received " + disconEvent.toString());
		}
		...
		if (session == null) {
			System.err
					.println("CMMqttEventHandler.processDISCONNECT(), session of client (" + strClient + ") is null!");
		}
...
//			session = mqttInfo.getMqttSessionHashtable().get(recEvent.getMqttSender());
		...
		if (session == null) {
			System.err.println("CMMqttEventHandler.processQoS3PUBREC(), session is null!");
			return false;
		}
		...
		if (strSysType.equals("CLIENT")) { //응답 이벤트 리스트에 방금 받은 pubrec 추가
			...
			if(eventSync.getMinNumWaitedEvents()<=eventSync.getSizeOfReplyEventList()) {
				...
				synchronized(eventSync)
				{
					try {
						eventSync.notify();
						System.out.println("CMMqttManager.processQoS3PUBREC(), notify");
					} catch (Exception e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
						return false;
			...
			String strMqttReceiver = relEvent.getReceiver();
			...
			if (bRet && CMInfo._CM_DEBUG) {
				System.out.println("CMMqttEventHandler.processQoS3PUBREC(), sent " + relEvent.toString());
			}
			if (!bRet) {
				System.err.println("CMMqttEventHandler.processQoS3PUBREC(), error to send " + relEvent.toString());
				return false;
			}
		} else if (strSysType.equals("SERVER")) {
			sendPUBREC(recEvent);
	...
	/*
	private boolean processQoS3PUBREL(CMMqttEvent event) {
		// The PUBLISH sender with QoS 3 sends PUBREL in response to PUBREC.
		CMMqttEventPUBREL relEvent = (CMMqttEventPUBREL) event;
		if (CMInfo._CM_DEBUG) {
...
//		System.out.println("process pubrel3");
		...
		if (session == null) {
			System.err.println("CMMqttEventHandler.processQoS3PUBREL(), session is null!");
			return false;
		}
		...
		if (strSysType.equals("CLIENT")) {
			...
			if (!bRet) {
				System.err.println("CMMqttEventHandler.processQoS3PUBREL(), error to delete " + "PUBLISH event with packet ID ("
						+ nPacketID + ") from the " + "recv-unack-publish list!");
				return false;
			}
			...
			if (bRet && CMInfo._CM_DEBUG) {
				System.out.println("CMMqttEventHandler.processQoS3PUBREL(), sent " + compEvent.toString());
			}
			if (!bRet) {
				System.err.println("CMMqttEventHandler.processQoS3PUBREL(), error to send " + compEvent.toString());
				return false;
			}
		} else if (strSysType.equals("SERVER")) {
			sendPUBREL(relEvent);
	...
	/*
	private boolean processQoS3PUBCOMP(CMMqttEvent event) {
		// The PUBLISH receiver with QoS 3 sends PUBCOMP in response to PUBREL.
		CMMqttEventPUBCOMP compEvent = (CMMqttEventPUBCOMP) event;
		if (CMInfo._CM_DEBUG) {
...
//			session = mqttInfo.getMqttSessionHashtable().get(compEvent.getMqttSender());
		...
		if (session == null) {
			System.err.println("CMMqttEventHandler.processQoS3PUBCOMP(), session is null!");
			return false;
		}
		...
		if (strSysType.equals("CLIENT")) {
			...
			// to delete PUBREC from the recv-unack-pubrec list
			int nPacketID = compEvent.getPacketID();
			String strMqttReceiver = compEvent.getSender();
